...
| `screen.json` | Sample screening configuration, also used by the dogfood screen. It sets thresholds only for the judgments that met the pre-registered criteria. |
...
| `cache/` | One file per request, named by the SHA-256 of the exact request. Each file holds the request (model, state, questions) and the service's response. It also holds the answers of the [dogfood screen](../../docs/guides/intent-screening.md#dogfood-screen), which uses `screen.json`. |
