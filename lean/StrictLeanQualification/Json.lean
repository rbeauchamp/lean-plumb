...
theorem validateDecoded_exact (decoded : Except String (List Check)) :
  ...
  | ok checks => simp [validateDecoded, Except.bind, StrictLean.ExecutableContract.run, evaluate_success]

/-- Registered contract used by report adapters, not an IO authenticity assertion. -/
theorem checkedDecoded : StrictLean.ExecutableContract validateDecoded
