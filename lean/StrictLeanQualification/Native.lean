...
namespace StrictLeanQualification.Native
...
/-- Strict Lean owns precisely the native named-error prefix used by the old harness. -/
def isNative (message : Message) : Bool := message.kind.startsWith "StrictLean.SL"
...
def nativeMatches (expected : Expected) (message : Message) : Bool :=
  ...
  message.data.contains "https://rbeauchamp.github.io/strict-lean/dev/rules/" &&
...
def NativeMatches (expected : Expected) (message : Message) : Prop :=
  ...
  message.data.contains "https://rbeauchamp.github.io/strict-lean/dev/rules/" = true ∧
  message.severity = expected.severity
...
theorem validate_exact (expected : Expected) (exitCode : Nat) (stderr : String)
  ...
  simp [validate, StrictLean.ExecutableContract.run, evaluate_success, Satisfied,
...
end StrictLeanQualification.Native
