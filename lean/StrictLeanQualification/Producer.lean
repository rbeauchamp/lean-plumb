...
/-! Exact producer-observation requirements for SL5001/SL5002 and standalone roots.
...
namespace StrictLeanQualification.Producer
open Lean
...
def requirements (report : Json) (code : Nat) (rule mode source : String)
  ...
  let material := if rule == "SL5002" then #[key] else #[]
  ...
  let mut checks : List Check := [
    ...
    ⟨"module documentation", (← field docs "modules") == toJson #[toJson #[nameJson "Example", .bool (rule == "SL5002" || fixed)]]⟩,
  ...
  if !fixed then
    ...
    checks := checks ++ [
      ...
      ⟨"location kind", (← text location "kind") == (if rule == "SL5001" then "module" else "source")⟩,
...
def standaloneRequirements (report : Json) (code : Nat) (mutated : Bool) : Except String (List Check) := do
  ...
  return [
    ...
    ⟨"standalone diagnostics", ids == (if mutated then ["SL1001"] else [])⟩,
...
end StrictLeanQualification.Producer
