...
import closure contains `Lean.Environment`, so `StrictLean.Diagnostic` supplies
...
def leanPosition (p : StrictLeanPolicy.Position) : Lean.Position := ⟨p.line, p.column⟩

/-- Byte offsets `FileMap` assigns to a reported range's ends. -/
def reportedBytes (fm : FileMap) (r : StrictLeanPolicy.Range) : ByteRange :=
...
def ReportedAgrees (column : Utf16Column) (fm : FileMap) (r : StrictLeanPolicy.Range) : Prop :=
...
instance (column : Utf16Column) (fm : FileMap) (r : StrictLeanPolicy.Range) :
    Decidable (ReportedAgrees column fm r) :=
  inferInstanceAs (Decidable (_ ∧ _))

/-- The candidate a report's full and selection ranges name in `snapshot`. -/
def reportedCandidate (snapshot : SourceSnapshot) (ranges : StrictLeanPolicy.Ranges) :
    SourceCandidate :=
...
def reportedRange (column : Utf16Column) (fm : FileMap) (r : StrictLeanPolicy.Range) :
...
theorem reportedRange_eq (column : Utf16Column) (fm : FileMap) (r : StrictLeanPolicy.Range) :
...
def sourceFromReportWith (column : Utf16Column) (snapshot : SourceSnapshot)
    (ranges : StrictLeanPolicy.Ranges) : Except String SourceLocation := do
