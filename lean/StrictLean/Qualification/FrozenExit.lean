...
namespace StrictLean.Qualification.FrozenExit
open Lean System StrictLeanQualification StrictLeanQualification.Evidence SourceEvidence
    ...
    for (binary, flags) in #[("docFenceAudit", #["--jobs", "1", "--verbose"]), ("axiomGate", #["--with-docs"])] do
      ...
      for phase in #["positive", "changed", "restored"] do
        ...
        check {
          ...
          fence := importSource bad action, ids := if bad then ["SL4002", "SL2005"] else [],
      ...
      check {
        ...
        fence := importSource true action, ids := ["SL4002", "SL2005"],
      ...
      check {
        ...
        fence := importSource true action, ids := ["SL4002"], reason := "checker inspection failed" }
    ...
    for (label, binary, flags) in #[
      ...
      for phase in #["positive", "missing", "restored", "ordinary-error", "ordinary-restored"] do
        ...
        check {
          ...
          ids := if missing then ["SL2005"] else if ordinary then [if binary == "axiomGate" then "SL2003" else "error"] else [],
...
end StrictLean.Qualification.FrozenExit
