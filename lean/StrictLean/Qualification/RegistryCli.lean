...
import StrictLeanQualification.Registry
...
The pure output oracle is `StrictLeanQualification.Registry.checkedValidation`.
This is operational evidence for the CLI, not a universal proof of `axiomGate`. -/

namespace StrictLean.Qualification.RegistryCli
open Lean System
...
def check : IO Unit := do
  ...
  withScratch root "registry-cli" fun scratch => do
    ...
    for arguments in cases root scratch output do
      ...
      match StrictLeanQualification.Registry.checkedValidation.run result.exitCode.toNat report with
      | .ok () => pure ()
...
end StrictLean.Qualification.RegistryCli
