...
public import StrictLean.Diagnostic
...
namespace StrictLean.Findings
open Lean

/-- New diagnostic identity is recovered from the original structural Name only. -/
def declarationName (decl : StrictLeanPolicy.Declaration) : Except String Name := do
...
def declarationLocation (decl : StrictLeanPolicy.Declaration) (snapshot : Option SourceSnapshot) :
