...
Canonical metadata motivation credits con-leche (StrictLean.RuleId). -/
open Lean StrictLean StrictLean.RegistryCodec
...
def main : IO Unit := do
  let producer := StrictLean.Checker.ResultProtocol.producer
  ...
  for id in RuleId.all do
    ...
    require (!succeeded (parseDescriptor ((descriptorJson id).setObjVal! "extra" .null)))
      s!"unknown field {id}"
  require (!succeeded (parseRule (.str "SL9999"))) "unknown rule"
  ...
  let compilation : StrictLean.Checker.SourceAudit.Compilation := {
  ...
  require (StrictLean.Checker.SourceAudit.sourceDiagnosticFailure compilation) "completed source diagnostic"
  require (!StrictLean.Checker.SourceAudit.sourceDiagnosticFailure
