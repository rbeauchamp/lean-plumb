...
namespace StrictLean.Checker.BuildLintQualification
...
private def witnessSource : String :=
  ...
  "theorem witnessContract : StrictLean.ExecutableContract witness " ++
  "(fun n => n.val = 1) := ⟨witness.property⟩\n"

private def identitySource (beforeDefinition : String := "") : String :=
  "import StrictLean.Contract\n" ++ beforeDefinition ++
  "def identity (n : Nat) := n\n" ++
  "theorem identityContract : StrictLean.ExecutableContract identity " ++
...
private def cases : Array Case := #[
  ...
  { name := "type-producing-root-alias"
    files := #[("Widget.lean", "import StrictLean.Contract\n" ++
      "@[irreducible] def ResultAlias := Nat\n@[macro_inline] def root : ResultAlias := by unfold ResultAlias; exact 0\n" ++
      "theorem contract : StrictLean.ExecutableContract root (fun _ => True) := ⟨True.intro⟩\n")]
  ...
  { name := "aliased-contract-registration"
    files := #[("Widget.lean", "import StrictLean.Contract\n" ++
      "def root (n : Nat) := n\n" ++
      "@[irreducible] def Required : Prop := StrictLean.ExecutableContract root (fun _ => True)\n" ++
  ...
  { name := "imported-cached-csimp"
    files := #[("Widget.lean", "import Support\nimport StrictLean.Contract\n" ++
      "theorem importedContract : StrictLean.ExecutableContract Support.entry " ++
      "(fun f => ∀ n, f n = n) := ⟨fun _ => rfl⟩\n")]
    support := some ("namespace Support\ndef target (n : Nat) := n\n" ++
  ...
  { name := "imported-unsafe-replacement"
    files := #[("Widget.lean", "import Support\nimport StrictLean.Contract\n" ++
      "theorem importedContract : StrictLean.ExecutableContract Support.reference " ++
      "(fun f => ∀ n, f n = n) := ⟨fun _ => rfl⟩\n")]
...
private def setup (repo adopter : FilePath) : IO Unit := do
  ...
  let dependency := Json.mkObj [
    ("name", toJson "strict_lean"), ("scope", toJson ""),
...
end StrictLean.Checker.BuildLintQualification
