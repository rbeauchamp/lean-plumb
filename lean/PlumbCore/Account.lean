...
namespace Plumb.Checker.Account
...
def acceptanceTheorem : Lean.Name := ``PlumbPolicy.accept_iff
...
/-- PL1007 checks the registration's shape and Lean checks `R f`; whether `R` is the intended
...
theorem checkedAccount : Plumb.ExecutableContract @accountImpl AccountContract := by
  refine ⟨fun c run => ?_⟩
