...
open PlumbPolicy (Declaration)
open PlumbPolicy.Frontend (SyntaxRange Command Transcript)
...
def RangesConvert (column : Utf16Column) (snapshot : SourceSnapshot)
    (ranges : PlumbPolicy.Ranges) : Prop :=
...
def reportObligations (column : Utf16Column) (snapshot : SourceSnapshot)
    (ranges : PlumbPolicy.Ranges) : List Obligation :=
  [(ReportedAgrees column snapshot.source.toFileMap ranges.range,
...
theorem checkedCoordinates : Plumb.ExecutableContract coordinateCheck CoordinateContract :=
