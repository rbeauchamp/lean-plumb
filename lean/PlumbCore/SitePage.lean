import PlumbCore.Site
import PlumbCore.Guide

/-! # Rule-reference page sources

Pure construction of the Verso source of every generated page of the rule reference. The
operational builder (`Plumb.Site.Build`) supplies the checked example evidence and resolved
...
open Plumb.Checker.Account (Residual Trusted)
...
def editionHtml (ident : Identity) (route : String) : String :=
  ...
  "<aside class=\"plumb-edition\" aria-label=\"Documentation version\"><p><strong>Development documentation</strong> generated from " ++
...
private def preHtml (text : String) : String :=
  "<pre class=\"plumb-code\"><code>" ++ escape text ++ "</code></pre>"
...
def shownHtml (ident : Identity) (f : ShownFile) : String :=
  "<figure class=\"plumb-file\"><figcaption>" ++ fileCaption ident f ++ "</figcaption>" ++ preHtml f.text ++ "</figure>"
...
private def keepHtml (l : String) : String := "<span class=\"plumb-keep\">  " ++ escape l ++ "</span>\n"

private def foldHtml (n : Nat) : String := "<span class=\"plumb-keep\">  ⋯ " ++ toString n ++ " unchanged lines</span>\n"
...
def diffHtml (d : List DiffLine) : String :=
  let runs := diffRuns d
  "<pre class=\"plumb-diff\"><code>" ++ String.join ((List.range runs.length).zip runs |>.map fun (i, run) =>
    match run with
    | .inl ls => keepRunHtml 3 (i == 0) (i + 1 == runs.length) ls
    | .inr (.remove l) => "<del class=\"plumb-remove\">- " ++ escape l ++ "</del>\n"
    | .inr (.add l) => "<ins class=\"plumb-add\">+ " ++ escape l ++ "</ins>\n"
    | .inr (.keep l) => keepHtml l) ++ "</code></pre>"

def findingHtml (f : FindingView) : String :=
  "<div class=\"plumb-finding\"><p>" ++ code f.rule.spelling ++ " <span class=\"plumb-badge\">" ++ escape f.severity ++
  "</span> <span class=\"plumb-badge plumb-impact-" ++ escape f.impact ++ "\">" ++ escape f.impact ++ "</span> " ++
...
def exampleHtml (ident : Identity) (ex : Example) : Except String String := do
  ...
  return "<div class=\"plumb-example\">" ++
    "<p class=\"plumb-status\">" ++ escape (exampleKindText ex.kind) ++ "</p>" ++
...
private def radios (dimension legend : String) (values : List (String × String)) : String :=
  ...
  String.join ((("all", "All") :: values).map fun (value, label) =>
    "<span class=\"plumb-choice\"><input type=\"radio\" name=\"" ++ dimension ++ "\" id=\"" ++ radioId dimension value ++
...
private def selectionSelector (s : Selection) : String :=
  ".plumb-index:has(#" ++ radioId "category" (optionSlug RuleCategory.slug s.category) ++ ":checked):has(#" ++
...
def filterCss : String :=
  String.join (categories.map fun c => ".plumb-index:has(#" ++ radioId "category" c.slug ++
    ":checked) tr.plumb-rule:not(.category-" ++ c.slug ++ "){display:none}\n") ++
  String.join (modes.map fun m => ".plumb-index:has(#" ++ radioId "mode" m.spelling ++
    ":checked) tr.plumb-rule:not(.mode-" ++ m.spelling ++ "){display:none}\n") ++
  String.join (availabilities.map fun a => ".plumb-index:has(#" ++ radioId "availability" a.slug ++
    ":checked) tr.plumb-rule:not(.availability-" ++ a.slug ++ "){display:none}\n") ++
  String.join (emptySelections.map fun s => selectionSelector s ++ " tr.plumb-no-match{display:table-row}\n")

private def ruleRow (id : RuleId) (exampleKind : String) : String :=
  let d := descriptor id
  "<tr class=\"plumb-rule category-" ++ d.category.slug ++ " availability-" ++ d.availability.slug ++
  String.join (d.evidenceModes.map fun m => " mode-" ++ m.spelling) ++ "\">" ++
  "<th scope=\"row\"><a href=\"" ++ id.route ++ "\">" ++ code id.spelling ++ "</a></th>" ++
  "<td><a href=\"" ++ id.route ++ "\">" ++ escape d.title ++ "</a><br><span class=\"plumb-muted\">subreason " ++
  code d.applicability ++ "</span></td>" ++
  "<td>" ++ escape d.category.label ++ "<br><span class=\"plumb-muted\">" ++ escape d.scope.label ++ "</span></td>" ++
  "<td>" ++ escape (joinComma (d.evidenceModes.map modeLabel)) ++ "</td>" ++
  "<td>" ++ escape d.availability.label ++ "<br><span class=\"plumb-muted\">" ++ escape (lifecycleText d.lifecycle) ++
...
def indexHtml (exampleKind : RuleId → String) : String :=
  "<div class=\"plumb-index\"><form class=\"plumb-filters\" aria-label=\"Filter rules\" action=\"#\">" ++
  ...
  "<div class=\"plumb-scroll\" role=\"region\" aria-label=\"Rule catalogue\" tabindex=\"0\"><table class=\"plumb-rules\"><caption>All " ++ toString RuleId.all.length ++ " registered rules</caption><thead><tr>" ++
