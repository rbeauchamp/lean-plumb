...
def infrastructureModuleNames : Array Name :=
  reporterModuleNames ++ #[`Plumb.StructuralName, `Plumb.Collect]
...
def InfrastructureOK (c : Claim) (i : EnvironmentCensus) : Prop :=
  ...
  (∀ receipt ∈ i.infrastructure, receipt.moduleKey.name.name = `Plumb.Collect →
    ∀ origin ∈ i.origins, `Plumb.Collect ∈ origin.imports → origin.name ∈ reporterModuleNames) ∧
