...
def homePage (ident : Identity) : Except String String := do
  ...
  return header ["Generated.Rules", "Generated.Versions", "Generated.Credits"] "Regula rule reference" "regula" none ++
    notice ++ "\n" ++
    "Regula is a strict linter for Lean 4 projects. This reference explains every diagnostic it reports: what triggers it, why it matters, how to fix it, a checked example produced by the real checker, and exactly what a passing result does and does not establish.\n\n" ++
    "**If you followed a diagnostic link.** Every Regula diagnostic carries a stable rule ID such as `RG1001` (editor code `Regula.RG1001`) and ends with its explanation URL, for example `" ++ devUrl .projectAxiom ++ "`. In VS Code the infoview's *View explanation* link opens the same page. The page's ID is the diagnostic's ID.\n\n" ++
...
def versionsPage (ident : Identity) (evidence : List EvidenceSummary) : Except String String := do
  ...
  let identity ← htmlBlock ("<table class=\"regula-facts\"><caption>Build identity</caption><tbody>" ++
    ...
    "</tbody></table>")
  let table ← htmlBlock ("<div class=\"regula-scroll\" role=\"region\" aria-label=\"Checked rule examples\" tabindex=\"0\"><table class=\"regula-rules\"><caption>Checked rule examples of this build</caption><thead><tr>" ++
...
def creditsPage (ident : Identity) : Except String String := do
  ...
  return header [] "Credits and licenses" "credits" (some "credits") (split := false) ++ notice ++ "\n" ++
    "Regula and this site's generated content are released under the [MIT license](" ++ blobUrl ident "LICENSE" ++ "). The rule explanations are original.\n\n" ++
    "**Lean and Lake.** The linter, its editor messages and its Lake integration are built on Lean 4 and Lake by the Lean FRO and contributors (Apache 2.0), used through the pinned toolchain as dependencies. The editor's *View explanation* link is Lean's own error-description widget, shown in the infoview of the VS Code Lean 4 extension (leanprover/vscode-lean4). The linter's strict JSON parser adapts the container recursion of Lean's `Lean/Data/Json/Parser.lean` (Copyright (c) 2019 Gabriel Ebner; authors Gabriel Ebner and Marc Huisinga; Apache 2.0) to reject duplicate keys; its notice is kept in the source and the [Apache 2.0 license text](" ++ blobUrl ident "LICENSES/Apache-2.0.txt" ++ ") is in the repository.\n\n" ++
    "**Verso.** This site is generated with [Verso](https://github.com/leanprover/verso) by the Lean FRO and contributors (Apache 2.0), pinned at revision `" ++ ident.versoRevision ++ "` ([license](https://github.com/leanprover/verso/blob/" ++ ident.versoRevision ++ "/LICENSE)). The stylesheets and the search and table-of-contents scripts served with every page come from Verso. The separation of documentation and example toolchains follows David Thrane Christiansen's [package-docs template](https://github.com/leanprover/verso-templates/tree/76c9edf5a70f14d272af0f0f354ec833ac22c350/package-docs); no template text is copied. Verso bundles the third-party JavaScript components listed below with their licenses. Pages also load the [marked](https://github.com/markedjs/marked) Markdown library (MIT) from the jsDelivr CDN, as Verso configures by default.\n\n" ++
    "**con-leche.** The closed typed rule registry, its canonical metadata, the complete, request-indexed acceptance design and the proved equality of executed decisions with their reference definitions follow ideas from [con-leche](https://github.com/leanprover/con-leche/tree/c431b1ca1b7a93486dd3e0440d3ee82abe90ccd0) by Joachim Breitner and contributors (Lean FRO), in particular `Kernel/PropWhen.lean`, `Cached/Installed.lean` and `Frontend/Scan/Equiv.lean`. No con-leche code or proof is copied, and con-leche's theorems are not claimed for Regula. con-leche did not design this site.\n\n" ++
    "**Rule-page structure.** The page layout of cause, rationale, fix, configuration, examples and limitations follows [Microsoft's CA1416 code-analysis rule page](https://learn.microsoft.com/en-us/dotnet/fundamentals/code-analysis/quality-rules/ca1416) as one illustrative reference; no .NET content is used, and no affiliation with or endorsement by Microsoft is implied. Other linters' references (Clippy, ESLint, Ruff, HLint) informed the design as documented in the " ++
    "[ecosystem study](" ++ blobUrl ident "docs/guides/ecosystem-design.md" ++ ").\n\n" ++
    "**Mathlib.** The checker repository's audit surface depends on [Mathlib](https://github.com/leanprover-community/mathlib4) (Apache 2.0); the linter itself imports only Lean's core libraries. The repository's qualification and site tooling also use [Batteries](https://github.com/leanprover-community/batteries) (Apache 2.0), which Mathlib requires.\n\n" ++
    "Crediting a project does not imply its endorsement of Regula. The complete attribution account is in the " ++ "[design-influences guide](" ++ blobUrl ident "docs/guides/design-influences.md" ++ ").\n\n" ++
    "{licenseInfo}\n"
