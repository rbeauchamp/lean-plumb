...
```text
lean --run lean/Plumb/Site/Deployment.lean archive REPOSITORY DIR   # used by `lake exe site build`
lean --run lean/Plumb/Site/Deployment.lean gate ARTIFACT_DIR        # before archiving and deploying
lean --run lean/Plumb/Site/Deployment.lean verify ARTIFACT_DIR      # after deploying
```

The site archive is the branch `site-archive`. It holds only `rev/<commit>/` snapshot
...
`verify` requires `PLUMB_PAGE_URL` to be the artifact's recorded site, the live `build.json`
...
repository rule on `site-archive` would enforce it for every writer.
-/
...
/-- The branch that retains every published revision snapshot. -/
def archiveRef : String := "refs/heads/site-archive"

/-- A full lowercase hexadecimal commit identifier, as `Plumb.Site.IsCommit`. -/
...
def main (args : List String) : IO UInt32 := do
  ...
  | ["archive", repository, dir] =>
    let archive ← Plumb.Site.Deployment.fetchArchive repository dir
  ...
  | _ => IO.eprintln "usage: lean --run lean/Plumb/Site/Deployment.lean (archive REPOSITORY DIR | gate ARTIFACT_DIR | verify ARTIFACT_DIR)"; return 2
