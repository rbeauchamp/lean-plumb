...
Canonical metadata motivation credits con-leche (Plumb.RuleId). -/
open Lean Plumb Plumb.RegistryCodec
...
def main : IO Unit := do
  ...
  for id in RuleId.all do
    ...
    require (!succeeded (parseDescriptor ((descriptorJson id).setObjVal! "extra" .null)))
      s!"unknown field {id}"
  require (!succeeded (parseRule (.str "PL9999"))) "unknown rule"
  ...
  let compilation : Plumb.Checker.SourceAudit.Compilation := {
  ...
  require (Plumb.Checker.SourceAudit.sourceDiagnosticFailure compilation) "completed source diagnostic"
  require (!Plumb.Checker.SourceAudit.sourceDiagnosticFailure
