import Plumb.Checker.ProducerReport
import Lean.Replay
import Plumb.Probe
...
namespace Plumb.Checker.Admission
...
unsafe def validate (env : Environment) (ownedModules : Array Name) :
  ...
  let reporterModules := #[`Plumb.Probe, `Plumb.Report,
    `Plumb.Checker.PolicyCodec, `Plumb.StructuralName]
  ...
  let own := Plumb.Probe.ownedConstants env replayModules.toList
  let mut required := #[]
  ...
  try
    ...
    return .ok { modules := PlumbPolicy.canonicalNames replayModules, required, admitted }
  catch error =>
