...
Universal value/collection laws live in PlumbPolicy. These controls exercise
...
namespace Plumb.Checker.PolicyQualification
open Lean System
open Plumb.Checker
...
def transport : Array String := Id.run do
  ...
  let boundary : Plumb.Report.ExecutionBoundary := {
  ...
  let decode (j : Json) := (fromJson? j : Except String Plumb.Report.ExecutionBoundary)
  ...
  let .ok nativeOrigin := PlumbPolicy.admitNativeOrigin `Init
  ...
  let root : Plumb.Report.ExecutionRoot := {
  ...
  failures := failures ++ expectOk "execution-inventory-positive" (PlumbPolicy.admitExecution #[root])
  failures := failures ++ expectError "duplicate-root" "invalid execution inventory"
    (PlumbPolicy.admitExecution #[root, root])
  failures := failures ++ expectError "duplicate-boundary-occurrence" "invalid execution inventory"
    (PlumbPolicy.admitExecution #[{root with boundaries := #[boundary, boundary]}])
  failures := failures ++ expectOk "execution-inventory-restored" (PlumbPolicy.admitExecution #[root])
...
private def source : String :=
  ...
  "    Plumb.RegistryCodec.parseName (Plumb.RegistryCodec.nameJson n) = .ok n :=\n" ++
  "  Plumb.RegistryCodec.name_roundtrip n\n" ++
...
private def setup (repo adopter : FilePath) : IO Unit := do
  ...
  let dependency := Json.mkObj [("name", .str "plumb"), ("scope", .str ""),
...
def publicPaths (repo scratch : FilePath) : IO (Array String) := do
  ...
  let mut failures := #[]
  let cases := #[("forbidden-report", "import Plumb.Report\n" ++ source, #["probe", "Plumb.Report"]),
    ("forbidden-probe", "import Plumb.Probe\n" ++ source, #["probe", "Plumb.Probe"]),
...
def nativeImport (repo scratch : FilePath) : IO (Array String) := do
  let adopter := scratch / "adopter"
  setup repo adopter
  let nativeSource := "import Plumb.Linter\n" ++ source.replace "import Lean\n"
...
end Plumb.Checker.PolicyQualification
