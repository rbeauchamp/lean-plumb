...
namespace Plumb.Checker.AcceptanceLink
open Lean System
...
def identityJson (projectRoot docsRoot : FilePath)
    ...
    (documents : Array PlumbPolicy.SourceSnapshot) : Json :=
...
def identity (scratch projectRoot docsRoot : FilePath)
    ...
    (configuration : Array (FilePath × Option String))
    (dependencies : Array Snapshot.DependencyObservation)
    (documents : Array PlumbPolicy.SourceSnapshot) : IO String := do
...
end Plumb.Checker.AcceptanceLink
