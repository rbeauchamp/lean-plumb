...
namespace Plumb.Checker.Frontend
open scoped Plumb.Report

open Lean Lean.Elab
open PlumbPolicy (EvaluatorRole)
open Plumb.Checker.PolicyCodec (exactFields)

abbrev ImportRecord := PlumbPolicy.Frontend.ImportRecord
deriving instance ToJson for PlumbPolicy.Frontend.ImportRecord
instance : FromJson PlumbPolicy.Frontend.ImportRecord := ⟨fun j => do
...
abbrev SyntaxRange := PlumbPolicy.Frontend.SyntaxRange
deriving instance ToJson for PlumbPolicy.Frontend.SyntaxRange
instance : FromJson PlumbPolicy.Frontend.SyntaxRange := ⟨fun j => do
...
abbrev Evaluator := PlumbPolicy.Frontend.Evaluator
deriving instance ToJson for PlumbPolicy.Frontend.Evaluator
instance : FromJson PlumbPolicy.Frontend.Evaluator := ⟨fun j => do
...
abbrev AddedDeclaration := PlumbPolicy.Frontend.AddedDeclaration
deriving instance ToJson for PlumbPolicy.Frontend.AddedDeclaration
instance : FromJson PlumbPolicy.Frontend.AddedDeclaration := ⟨fun j => do
...
abbrev DeclarationBinding := PlumbPolicy.Frontend.DeclarationBinding
deriving instance ToJson for PlumbPolicy.Frontend.DeclarationBinding
instance : FromJson PlumbPolicy.Frontend.DeclarationBinding := ⟨fun j => do
...
abbrev Command := PlumbPolicy.Frontend.Command
deriving instance ToJson for PlumbPolicy.Frontend.Command
instance : FromJson PlumbPolicy.Frontend.Command := ⟨fun j => do
...
abbrev Transcript := PlumbPolicy.Frontend.Transcript
deriving instance ToJson for PlumbPolicy.Frontend.Transcript
instance : FromJson PlumbPolicy.Frontend.Transcript := ⟨fun j => do
...
def validateCoordinates (declarations : Array Plumb.Report.Declaration)
...
private unsafe def buildCore (moduleName : Name) (sourcePath : System.FilePath)
  ...
  let opts := (Lean.Elab.async.set (warningAsError.set {} true) false).setBool `linter.plumb false
...
instance : FromJson WorkerRequest := ⟨fun j => do
  Plumb.Checker.PolicyCodec.exactFields j ["moduleName", "source", "searchRoots"]
  return {
