...
namespace Plumb.Checker.FreshChecker
...
private unsafe def freezeGraph (plan : Plan) (snapshot : PlumbPolicy.AdmittedSnapshot)
  ...
  let modules ← names.mapM fun name => do
    pure (⟨snapshot, ← IO.ofExcept (PlumbPolicy.admitIdentity name)⟩ : PlumbPolicy.ModuleKey)
  let keyFor (name : String) : IO PlumbPolicy.ModuleKey := do
  ...
  let moduleSources ← modules.mapM fun key => do
    ...
    pure (key, (⟨source.path, source.content⟩ : PlumbPolicy.SourceSnapshot))
  ...
  let origins ← try
      ...
      modules.mapM fun key => do
        ...
        pure (⟨key.name.name, path.toString, data.imports.map (·.module)⟩ : PlumbPolicy.ModuleOrigin)
    finally Lean.searchPathRef.set previous
  let policy ← IO.ofExcept <| PlumbPolicy.admitInventory #[] #[]
  let execution ← IO.ofExcept <| PlumbPolicy.admitExecution #[]
  let request : PlumbPolicy.EnvironmentRequest := { key := ⟨snapshot, 0⟩, modules }
  let environment : PlumbPolicy.EnvironmentCensus := {
  ...
  let admitted ← IO.ofExcept <| PlumbPolicy.buildPlan claim census
  return ⟨claim, census, admitted, fun slot => PlumbPolicy.authorize census.environments[slot].policy⟩

private def finishGraph (frozen : FrozenGraph) (build : ProcessResult) (checks : Array Check) :
    Except String (PlumbPolicy.AcceptedRun frozen.claim) := do
  let checked ← checks.mapM fun check => do
    ...
    pure key
  let graph : PlumbPolicy.GraphObservation := {
  ...
  let inputs ← frozen.plan.jobs.mapIdxM fun slot key => do
    let evidence ← match key.stage, key.subject with
      | .configuration, .scope => pure (PlumbPolicy.JobEvidence.configuration frozen.census.configuredTargets frozen.census.discoveredTargets)
    ...
    pure (slot, ({ key, snapshot := frozen.claim.val.snapshot, completion := .completed, evidence } : PlumbPolicy.JobObservation))
  let result ← (PlumbPolicy.finalize frozen.plan frozen.roles inputs.toList).mapError
...
unsafe def run (args : List String) : IO UInt32 := do
  ...
  let mut accepted : Option ((c : PlumbPolicy.Claim) × PlumbPolicy.AcceptedRun c) := none
...
end Plumb.Checker.FreshChecker

unsafe def main (args : List String) : IO UInt32 := do
  try
    Plumb.Checker.initializeLeanSearchPath
    Plumb.Checker.FreshChecker.run args
  catch error =>
