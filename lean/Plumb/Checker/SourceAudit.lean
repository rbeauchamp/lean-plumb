...
namespace Plumb.Checker.SourceAudit
...
instance : FromJson SourceSpec := ⟨fun j => do
  Plumb.Checker.PolicyCodec.exactFields j ["module", "source", "warningAsError", "rejectWarnings", "captureRejection"]
  return {
...
instance : FromJson Compilation := ⟨fun j => do
  Plumb.Checker.PolicyCodec.exactFields j ["spec", "sourcePath", "oleanPath", "ileanPath", "process", "errors"]
  return {
...
instance : FromJson GroupRequest := ⟨fun j => do
  Plumb.Checker.PolicyCodec.exactFields j ["modules", "sourceBindings", "ownedOutput", "includeExecution", "includeModuleOrigins"]
  return {
  ...
  SourceBinding.withUnchanged #[{
    ...
    if spec.captureRejection then
      ...
      let process ← spawn binary.toString
        #["--diagnostic-worker", (Plumb.RegistryCodec.nameJson spec.module.toName).compress, sourcePath.toString, output.toString]
...
instance : FromJson CompileBatch := ⟨fun j => do
  Plumb.Checker.PolicyCodec.exactFields j ["scratch", "jobs", "specs"]
  return {
