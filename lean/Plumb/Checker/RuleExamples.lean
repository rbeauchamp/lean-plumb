...
namespace Plumb.Checker.RuleExamples
open Lean System
...
unsafe def documentation (repo docsRoot output : FilePath) : IO UInt32 := do
  ...
  let outcome ← (stable #[] requestedConfiguration <| withScratch repo "rule-document-example" fun scratch => do
    ...
    stable #[] configuration do
      ...
      stable projectSources configuration do
        ...
        let certificate ← IO.mkRef (none : Option ((c : PlumbPolicy.Claim) × PlumbPolicy.AcceptedRun c))
  ...
  let completion ← if code == 0 then do
      ...
      let account := Plumb.Checker.Account.account accepted
      let fences := account.val.fences
      pure (if fences.positive > 0 && fences.compilerRejection + fences.policyRejection +
          fences.trustedTeaching == 0 then Plumb.Checker.Account.Status.completed account else .classified)
...
unsafe def main (args : List String) : IO UInt32 := do
  ...
  | "--injected-git-facts" :: facts :: rest =>
    let loaded ← (do
      ...
      IO.ofExcept (Lean.fromJson? (α := Array Plumb.Checker.Snapshot.GitFacts) json)).toBaseIO
    match loaded with
    | .error error =>
    ...
    | .ok table =>
      if table.isEmpty then
        ...
        return 2
      Plumb.Checker.Snapshot.injectedGitFacts.set table
      let (code, output) ← match rest with
        | "axiomGate" :: gateArgs =>
          pure (← Plumb.Checker.AxiomGate.entry gateArgs,
