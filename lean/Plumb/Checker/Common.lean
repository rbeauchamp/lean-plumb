...
import PlumbPolicy.ResultState
import Lean
...
def checkerPackageLibDir : IO (Option FilePath) := do
  ...
  if ← (libDir / "Plumb" / "Probe.olean").pathExists then
    return some libDir
  return none
...
def readJson (path : FilePath) : IO Json := do
  ...
  IO.ofExcept <| Plumb.Checker.PolicyCodec.parse text
...
def parseJsonOutput (what : String) (result : ProcessResult) : IO Json := do
  ...
  match Plumb.Checker.PolicyCodec.parse result.stdout with
...
def mapWorkQueue (jobs : Nat) (items : Array α)
  ...
  IO.ofExcept <| (PlumbPolicy.checkedIndexedResults.run items.size (fun _ _ => true)
...
def sourceWorkerRequest (stage : String) (moduleName : Name) (path : FilePath) (content : String) : Json :=
  Json.mkObj [("stage", .str stage), ("module", Plumb.RegistryCodec.nameJson moduleName),
...
def workerPacket (request payload : Json) : Json := Json.mkObj [
  ("schema", toJson (1 : Nat)),
  ("producer", Json.mkObj (Plumb.RegistryCodec.identityFields Plumb.Checker.Producer.identity ++ [("compilerCommit", .str Lean.githash)])),
  ("request", request), ("payload", payload)]

def readWorkerPacket (request packet : Json) : Except String Json := do
  Plumb.Checker.PolicyCodec.exactFields packet ["schema", "producer", "request", "payload"]
  unless (← packet.getObjValAs? Nat "schema") == 1 do throw "unsupported worker schema"
  unless (← packet.getObjVal? "producer") ==
      Json.mkObj (Plumb.RegistryCodec.identityFields Plumb.Checker.Producer.identity ++ [("compilerCommit", .str Lean.githash)]) do
...
def admitIndexedWorkerResults {α : Type} [FromJson α] (count : Nat) (binding : Nat → α → Bool)
  ...
  (PlumbPolicy.checkedIndexedResults.run count binding responses.toList).mapError fun
