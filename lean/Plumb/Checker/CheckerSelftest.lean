...
namespace Plumb.Checker.CheckerSelftest

open Lean System
open Plumb.Checker
open Plumb.Checker.Documentation
...
private def renderFileAudit (fixture : FixtureSpec) (_moduleName : String)
    (declarations : Array Plumb.Report.Declaration)
    (roots : Array Plumb.Report.ExecutionRoot)
...
private unsafe def fixtureVerdicts (repo scratch : FilePath) (jobs : Nat)
      ...
      pending := pending.push {
        ...
        wantsTranscript := moduleData.constants.any fun info =>
          Policy.declarationNeedsTranscript info.isUnsafe info.isPartial
            (Plumb.Probe.kindOf info) info.name
      }
    ...
    mapConcurrent jobs pending fun item => do
      ...
      try
        ...
        let spawned ← runProcess repo
          ...
          #["--transcript-worker", (Plumb.RegistryCodec.nameJson moduleName.toName).compress, item.compilation.sourcePath.toString,
            out.toString]
          #[("LEAN_PATH", some leanPathEnv)]
        if !spawned.succeeded then return (moduleName, none)
        match Plumb.Checker.PolicyCodec.parse (← IO.FS.readFile out) with
  ...
  let mut inspected : Array (CompiledFixture × Array Plumb.Report.Declaration ×
      Array Plumb.Report.ExecutionRoot) := #[]
...
private def manifestQualification (repo scratch : FilePath) : IO (Array String) := do
  ...
  IO.FS.writeFile unknownLibrary <| "{\"schema-version\":2,\"surfaces\":[{" ++
    ...
    "\"excluded-libraries\":[{\"library\":\"Fixtures\",\"rationale\":\"mutations\"}," ++
    "{\"library\":\"Plumb\",\"rationale\":\"checker\"}]," ++
...
private def structuralClaims : Array String := #["AuditApp", "PlumbPolicy"]

/-- Manifest claimed inside every structural copy: the actual repository manifest's
`AuditApp` surface and the `PlumbPolicy` surface, with every other actual library
...
private unsafe def structuralPartA (layout : SourceLayout) (repo copy : FilePath) : IO (Array String) := do
  ...
  withReplacedFile appRoot probeContaminated do
    if let some failure := expectedFailure "probe-contamination" (← gate)
        #["unexpected-project-module", "Plumb.Report"] then
      failures.modify (·.push failure)
...
private def adopterLeanLakefile (checkerPath : String) : String :=
  ...
  s!"require «plumb» from \"{checkerPath}\"\n\n" ++
...
unsafe def run (args : List String) : IO UInt32 := do
  ...
  if args == ["--native-adopter-only"] then
    let repo ← repoRoot
    let build ← runProcess repo "lake" #["build", "axiomGate", "Plumb.Linter"]
    if !build.succeeded then IO.println build.output; return 1
...
end Plumb.Checker.CheckerSelftest

unsafe def main (args : List String) : IO UInt32 := do
  try
    Plumb.Checker.initializeLeanSearchPath
    Lean.enableInitializersExecution
    Plumb.Checker.CheckerSelftest.run args
  catch error =>
