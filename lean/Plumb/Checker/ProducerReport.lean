...
abbrev Census := Plumb.Report.Census
deriving instance ToJson for Plumb.Report.Census
...
structure Environment extends Plumb.Report.Collected where
...
theorem Environment.resultJson_imports_independent (r : Environment) (modules : Array Name)
    (moduleOrigins : Array PlumbPolicy.ModuleOrigin) :
...
run_cmd do
  for name in #[``Plumb.Checker.ProducerReport.validate_eq_ok,
