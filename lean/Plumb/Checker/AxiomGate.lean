...
import Plumb.Checker.AcceptanceLink
...
namespace Plumb.Checker.AxiomGate
...
private def candidateModules (decls : Array Plumb.Report.Declaration) : Array Name :=
  Id.run do
...
private def reportContextFailure (id : Plumb.RuleId) (scope : String)
    (mode : Plumb.EvidenceMode) (impact : Plumb.Impact) (detail : String)
...
private def withSourceEvidenceOr (refused : α) (sources : Array ProducerReport.SourceBinding)
    ...
    (mode : Plumb.EvidenceMode) (composed : IO.Ref (Option Json)) (resultOut : Option FilePath) (action : IO α) : IO α := do
...
private def withSourceEvidence (sources : Array ProducerReport.SourceBinding)
    (configuration : Array (FilePath × Option String)) (scope : String)
    (mode : Plumb.EvidenceMode) (composed : IO.Ref (Option Json)) (resultOut : Option FilePath) (action : IO UInt32) : IO UInt32 :=
...
private structure ProjectEvidence where
  ...
  build : PlumbPolicy.BuildObservation
  claim : PlumbPolicy.Claim
  accepted : PlumbPolicy.AcceptedRun claim
...
private unsafe def auditSurfaceAt (repo manifestPath : FilePath)
  ...
  withSourceEvidence #[] configuration reportRoot.toString
    ...
    withSourceEvidence sourceBindings configuration reportRoot.toString
        (if fresh then .freshProject else .incrementalProject) composed resultOut do
      let snapshotFor (name : Name) : Option Plumb.SourceSnapshot :=
      ...
      let freezeRequest : IO (PlumbPolicy.AdmittedSnapshot ×
        ...
        let request ← IO.ofExcept <| PlumbPolicy.admitClaim {
      ...
      let mut findings : Array Plumb.Finding := #[]
      ...
      for (surface, outcome) in inspections do
        ...
        let mode : Plumb.EvidenceMode := if fresh then .freshProject else .incrementalProject
        ...
        for (moduleName, present) in documentation.modules do
          unless present do
            let finding ← IO.ofExcept <| Plumb.makeDiagnostic .moduleDocumentation
        ...
        for failure in Policy.executionFailureRecords executionInventory surface.execution do
          let location ← match report.declarations.find? (·.name == failure.root.name) with
            ...
            | none => pure (Plumb.Location.module failure.root.module)
      ...
      let accepted : Option (PlumbPolicy.AdmittedSnapshot ×
          ((c : PlumbPolicy.Claim) × PlumbPolicy.AcceptedRun c)) ←
...
private unsafe def auditSurface (repo : FilePath) (manifest : Option FilePath)
  ...
  if incremental then do
    ...
    let docFindings ← IO.mkRef (#[] : Array Plumb.Finding)
    let documentAccepted ← IO.mkRef (none : Option ((c : PlumbPolicy.Claim) × PlumbPolicy.AcceptedRun c))
    ...
    let combined ← if docsResult == 0 then do
        ...
        let receipt ← IO.ofExcept <| PlumbPolicy.combineAccepted documents evidence.accepted accepted
        ...
        | .ok (.ok inspected) =>
            ...
            for h : decl in scope.inventory.declarations do
              ...
              let verdict := match rule with
                | none => "OK"
                | some id => s!"VIOLATION[{(Plumb.descriptor id).applicability}]"
              IO.println s!"[{verdict}] {classification}"
              if let some id := rule then
                reasons := reasons.push (Plumb.descriptor id).applicability
            ...
            let accepted ← if reasons.isEmpty then
                ...
                | some .kernelOnly | some .choiceFree | some .standardLogical => do
                    ...
                    let actual : PlumbPolicy.SourceSnapshot := ⟨compiledSource.path, compiledSource.content⟩
                    let binding ← IO.ofExcept <| PlumbPolicy.admitFileSourceBinding requested actual
                    ...
                    pure (some (⟨request, accepted⟩ : (c : PlumbPolicy.Claim) × PlumbPolicy.AcceptedRun c))
                | _ => pure none
              else pure none
            if let some output := jsonOut then
...
unsafe def run (args : List String) : IO UInt32 := do
  ...
  if let ["--validate-site", registryPath, artifactPath] := args then
    ...
    IO.ofExcept <| Plumb.Website.validateArtifact ResultProtocol.producer registry artifact
    return 0
  if let ["--registry-out", output] := args then
    writeJson output (Plumb.RegistryCodec.registryJson ResultProtocol.producer)
    return 0
  if let ["--validate-registry", input] := args then
    let value ← IO.ofExcept <| Plumb.Checker.PolicyCodec.parse (← IO.FS.readFile input)
    IO.ofExcept <| Plumb.RegistryCodec.validateRegistry ResultProtocol.producer value
    return 0
  if let ["--declaration-report-worker", input, out] := args then
    let json ← IO.ofExcept <| Plumb.Checker.PolicyCodec.parse (← IO.FS.readFile input)
  ...
  if let ["--diagnostic-worker", moduleWire, source, out] := args then
    let moduleName ← IO.ofExcept <| Plumb.RegistryCodec.parseName (← IO.ofExcept <| Plumb.Checker.PolicyCodec.parse moduleWire)
  ...
  let reportFailure : IO.Error → IO UInt32 := fun error => do
    composed.set none
    let mode : Plumb.EvidenceMode := if options.file.isSome then .freshFile
