import Plumb.Checker.RuleExampleProjection
...
open Lean Plumb.Checker.RuleExampleQualification
...
private def corpusBody (json : Json) : Except String Unit := do
  ...
  let selected ← (← (← field json "selected").getArr?).mapM Plumb.RegistryCodec.parseRule
