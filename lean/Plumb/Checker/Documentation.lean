import Plumb.Checker.Acceptance
import PlumbPolicy.Pattern
import Plumb.Checker.SourceAudit
...
namespace Plumb.Checker.Documentation

open Lean System
open Plumb.Checker
open Plumb.Checker.Policy
...
structure Fence where
  document : PlumbPolicy.SourceSnapshot
  opening : PlumbPolicy.ByteRange
  bodyRange : PlumbPolicy.ByteRange
  closing : PlumbPolicy.ByteRange
  body : String
  line : Nat
...
def matchesPattern (pattern output : String) : Bool :=
  PlumbPolicy.matchesPattern pattern output
...
def scan (text origin : String) (sourceURI : Option String := none) : ScanResult := Id.run do
  ...
  let mut openInfo := ""
  let mut openLine := 0
  let mut opening : PlumbPolicy.ByteRange := ⟨0, 0⟩
  let mut bodyStart := 0
  let mut offset := 0
...
structure Result where
  ...
  status : Status
  detail : String := ""
  policyProblems : Array (Plumb.RuleId × Plumb.Report.Declaration) := #[]
...
private def assessPositive (task : Task) (unitName : Name) (declarations : Array Plumb.Report.Declaration)
  ...
  let (problems, policyProblems, compilerCount) := Id.run do
    ...
    for h : decl in scope.inventory.declarations do
      ...
      if let some id := Policy.ruleForMember decl (some claim) scope h then
        let reason := (Plumb.descriptor id).applicability
...
unsafe def auditTasks (repo scratch : FilePath) (jobs : Nat)
  ...
  withSourceEvidence tasks sourceBindings configuration do
    ...
    withSourceEvidence tasks snippets #[] do
      ...
      let complete ← IO.ofExcept <| (PlumbPolicy.checkedIndexedResults.run tasks.size
...
def Fence.key (fence : Fence) : Except String PlumbPolicy.FenceKey := do
...
structure DocumentPlan (claim : PlumbPolicy.Claim) where
  census : PlumbPolicy.Census
  plan : PlumbPolicy.Plan claim census
  roles : PlumbPolicy.CensusRoles census

def freezeDocuments (claim : PlumbPolicy.Claim) (tasks : Array Task) :
    Except String (DocumentPlan claim) := do
  let fences ← tasks.mapM (·.fence.key)
  let census : PlumbPolicy.Census := {
    requests := #[], environments := #[], modules := #[], moduleSources := #[],
    fences, configuredTargets := #[], discoveredTargets := #[] }
  let plan ← PlumbPolicy.buildPlan claim census
  return ⟨census, plan, fun slot => PlumbPolicy.authorize census.environments[slot].policy⟩
...
def exampleObservation (result : Result) : IO PlumbPolicy.ExampleObservation := do
  ...
  let units ← raw.units.mapM fun (task, compilation) => do
    ...
    pure ({
      ...
      source := ⟨compilation.sourcePath.toString, compilation.spec.source⟩ } : PlumbPolicy.ExampleUnit)
  let (census, outcome) ← if result.task.kind == .negative then do
      ...
      pure (#[], PlumbPolicy.ExampleOutcome.compilerRejection errors)
    else do
      ...
      pure (group.report.census.declarations,
        PlumbPolicy.ExampleOutcome.elaborated scope.inventory replay.required replay.admitted #[])
  return {
...
private def finishDocuments {claim : PlumbPolicy.Claim} (frozen : DocumentPlan claim)
    (build : PlumbPolicy.BuildObservation) (documents : Array PlumbPolicy.SourceSnapshot)
    (structural : Array String) (results : Array Result) : IO (PlumbPolicy.AcceptedRun claim) := do
  ...
  let inputs ← frozen.plan.jobs.mapIdxM fun slot key => do
    let evidence ← match key.stage, key.subject with
      | .discovery, .scope => pure (PlumbPolicy.JobEvidence.discovery frozen.census)
    ...
    pure (slot, ({ key, snapshot := claim.val.snapshot, completion := .completed, evidence } : PlumbPolicy.JobObservation))
  let result ← IO.ofExcept <| (PlumbPolicy.finalize frozen.plan frozen.roles inputs.toList).mapError
...
def captureMarkdown (root : FilePath) : IO (Array PlumbPolicy.SourceSnapshot) := do
...
def checkMarkdown (root : FilePath) (documents : Array PlumbPolicy.SourceSnapshot) : IO Unit := do
...
unsafe def auditBuiltProject (repo docsRoot : FilePath) (inventory : Lake.SurfaceInventory)
    ...
    (documents : Array PlumbPolicy.SourceSnapshot)
    (build : PlumbPolicy.BuildObservation) (jobs : Nat) (verbose : Bool)
    (emit : Plumb.Finding → IO Unit := fun _ => pure ())
    (observe : Array Result → IO Unit := fun _ => pure ())
    (observeAccepted : (claim : PlumbPolicy.Claim) → PlumbPolicy.AcceptedRun claim → IO Unit := fun _ _ => pure ())
    (sharedSnapshot : Option PlumbPolicy.AdmittedSnapshot := none) : IO UInt32 := do
  let outcome : Except ProducerReport.AdmissionFailure UInt32 ←
    SourceBinding.withUnchanged sourceBindings configuration do
      ...
      let claim ← IO.ofExcept <| PlumbPolicy.admitClaim {
      ...
      for result in results.qsort fun left right => left.task.origin < right.task.origin do
        ...
        if result.status == .fail then
          ...
          IO.println s!"      {detail}"
          let id : Plumb.RuleId := match result.task.kind with
          ...
          for (rule, decl) in result.policyProblems do
            ...
            let snapshot : Plumb.SourceSnapshot := {
...
end Plumb.Checker.Documentation
