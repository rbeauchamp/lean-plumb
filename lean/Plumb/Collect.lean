...
def declaration (name : Name) (stage : Stage) :
    CommandElabM PlumbPolicy.Declaration := do
  let env ← getEnv
  ...
  return {
    ...
    all := all.toArray
    hints
    valueConstants := PlumbPolicy.canonicalNames valueConstants
...
def currentModule (stage : Stage := .snapshot) : CommandElabM (Array PlumbPolicy.Declaration) := do
...
def commandDeclarations : CommandElabM (Array PlumbPolicy.Declaration) := do
  let env ← getEnv
