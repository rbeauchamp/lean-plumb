...
public import Plumb.Diagnostic
...
namespace Plumb.Findings
open Lean

/-- New diagnostic identity is recovered from the original structural Name only. -/
def declarationName (decl : PlumbPolicy.Declaration) : Except String Name := do
...
def declarationLocation (decl : PlumbPolicy.Declaration) (snapshot : Option SourceSnapshot) :
