...
import Plumb.Qualification.Slot
import PlumbQualification.Evidence
import PlumbQualification.Template
import PlumbQualification.Producer
import PlumbQualification.CorpusWindow
import Plumb.Checker.RuleExampleCorpusProjection
...
`PlumbQualification.CorpusWindow` state; the final `COMPLETED` export requires
...
tasks stay uncached. -/
namespace Plumb.Qualification.RuleExamples
open Lean System PlumbQualification.Evidence
...
private def produce (ctx : Context) (slot : Slot.ProducerSlot) (rule phase : String)
  ...
  if mode == "documentation" then
    ...
    if rule == "PL1002" && case == "Violation" then
      binary := root / ".lake/build/bin/ruleExamples"
  ...
  let requestKind := if rule == "PL1002" && case == "Violation" then "policyNegative"
  ...
  let expected ← if case == "Fixed" then pure (toJson (#[] : Array Json)) else do
    ...
    pure (← IO.ofExcept (PlumbQualification.Template.instantiate
      (PlumbQualification.Template.replace replacements) 64 template)).val
  let record := Plumb.Checker.RuleExampleProjection.record [
    ...
    ("stdout", .str execution.stdout), ("stderr", .str execution.stderr), ("detectorMillis", toJson elapsed)]
    (Plumb.Checker.RuleExampleProjection.resultView observed)
...
/-- PL5001 and PL5002 always land in the same shard while PL5001 is in the corpus. -/
theorem pl5001_pl5002_same_shard (keys : Array String) (count : Nat)
...
private def saveCompleted (evidence : FilePath) (fields : List (String × Json))
  ...
  save evidence (Plumb.Checker.RuleExampleProjection.corpus
...
def check (evidence : FilePath) (selection : Option (Array String))
  ...
  let ((finalFields, records), cleaned) ← withScratchCleaned root "rule-examples" fun scratch => do
    ...
    let depObservations ← Plumb.Checker.Snapshot.dependencies inventory
    ...
    let onceCaptures ← Plumb.Checker.Snapshot.dependenciesCaptures inventory
    ...
    save (rawDirectory / "injected-git-facts.json")
      (toJson (onceCaptures.map Plumb.Checker.Snapshot.DependencyCaptures.gitFacts))
    ...
    let productions := (PlumbQualification.CorpusWindow.productions selected.toList).toArray
    let recordCount := (PlumbQualification.CorpusWindow.records selected.toList).length
      ...
      match rule, phase with
        | "PL1005", "WrongClaim" =>
          produce ctx slot rule phase
            (some (← IO.FS.readFile (root / "examples/rules/PL1005/Violation.lean")))
            (some "standard-logical")
        | "PL4004", "TrustedControl" =>
          produce ctx slot rule phase (some ("<!-- lean-trusted-compiler -->\n```lean\n" ++
            (← IO.FS.readFile (root / "examples/rules/PL1004/Violation.lean")) ++ "```\n"))
        | "PL4004", "NegativeControl" =>
          produce ctx slot rule phase
    ...
    let initial := PlumbQualification.CorpusWindow.init producerWidth total
    ...
    try
      ...
      for _ in [:total] do
        ...
        if let some job := PlumbQualification.CorpusWindow.refill window then
          launch job
        window := PlumbQualification.CorpusWindow.consume window
        if index < recordCount then
    ...
    for record in records do
      ...
      let account ← IO.ofExcept (PlumbQualification.Producer.account report)
      ...
      IO.ofExcept (PlumbQualification.Producer.checkedValidation.run report
    ...
    save evidence (Plumb.Checker.RuleExampleProjection.corpus (("outcome", .str "INCOMPLETE") :: finalFields) records)
    ...
    Plumb.Checker.Snapshot.inputsUnchanged inventory
      (onceCaptures.map Plumb.Checker.Snapshot.observe)
...
end Plumb.Qualification.RuleExamples
