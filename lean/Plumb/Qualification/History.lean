import Plumb.Qualification.SourceEvidence
import PlumbQualification.History
    ...
    for (invocation, flags, mode) in #[
      ...
      for index in [:phases.size] do
        ...
        if phase == "admission" || phase == "source-change" then
          ...
          IO.ofExcept (PlumbQualification.Evidence.checkedValidation.run {
            failure := true, mode := some mode, status := "incomplete", ids := ["PL2005"], reason,
          ...
          if phase == "admission" then
            ...
            for d in ds do
              requireChecks [⟨"intended unchecked theorem", (← IO.ofExcept (PlumbQualification.Evidence.detail d)).contains "admissionFalse"⟩]
          IO.println s!"source {invocation}/{phase}: exact PL2005 incomplete PASS"
          continue
        IO.ofExcept (PlumbQualification.History.checkedValidation.run report result.exitCode.toNat
...
end Plumb.Qualification.History
