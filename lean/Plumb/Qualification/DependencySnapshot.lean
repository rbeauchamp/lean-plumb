...
/-! Native retention of dependency snapshot and PL3001 history controls. These
observations qualify trusted Lake/filesystem boundaries, not universal IO behavior. -/
namespace Plumb.Qualification.DependencySnapshot
open Lean System
...
private def control (project dependency : FilePath) : IO Unit := do
  let inventory ← Plumb.Checker.Lake.surfaceInventory project
  let before ← Plumb.Checker.Snapshot.dependencies inventory
  ...
  let unchanged := Plumb.Checker.Snapshot.inputsUnchanged inventory before
  unchanged
  let snapshot ← IO.ofExcept <| Plumb.Checker.Snapshot.make inventory.root #[] #[] before
  ...
  unchanged
  let built ← Plumb.Checker.Lake.buildTargets project #["Example"]
...
private def currentDirty (root : FilePath) (paths : Array FilePath) : IO String := do
  match ← (Plumb.Checker.Snapshot.observeGitFacts "control" root #[] paths).toBaseIO with
  ...
  withScratch root "snapshot-history" fun scratch => do
    ...
    for (mode, flags) in #[("fresh", #[]), ("incremental", #["--incremental"]), ("build-lint", #["--build-lint"])] do
      for (phase, fixture) in #[("positive", "Fixed.lean"), ("negative", "Violation.lean"), ("restored", "Fixed.lean")] do
        IO.FS.writeBinFile (project / "Example.lean") (← IO.FS.readBinFile (root / "examples/rules/PL3001" / fixture))
        ...
        if phase != "negative" then
          ...
          let historyFindings := findings.filter (fun d => (d.getObjValAs? String "id").toOption == some "PL3001")
          requireChecks [⟨"history incomplete", result.exitCode != 0 &&
            ...
            ⟨"not setup failure", !findings.any (fun d => (d.getObjValAs? String "id").toOption == some "PL2001")⟩,
...
end Plumb.Qualification.DependencySnapshot
