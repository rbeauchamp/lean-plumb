import Plumb.Qualification.InputInventory
...
namespace Plumb.Qualification.Acceptance
...
def worker (args : List String) : IO UInt32 := do
  let mode := (← IO.getEnv "PLUMB_PACKET_FAULT").getD ""
...
private def qualify (group : String) (evidence : FilePath) (receipt : IO.Ref Json) : IO Unit := do
  ...
  withScratch root "acceptance-controls" fun scratch => do
    ...
    IO.FS.writeFile (project / "Example.lean") "import Plumb.Contract\n/-! Public acceptance transport control. -/\ndef value : Nat := 7\n"
    ...
    for (phase, fault, reason) in cases do
      ...
      let env := cleanEnv ++ #[("PLUMB_QUALIFICATION_WRAPPER", some "acceptance"),
        ("PLUMB_PACKET_FAULT", some fault)]
...
end Plumb.Qualification.Acceptance
