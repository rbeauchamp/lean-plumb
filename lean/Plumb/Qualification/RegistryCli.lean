...
import PlumbQualification.Registry
...
The pure output oracle is `PlumbQualification.Registry.checkedValidation`.
This is operational evidence for the CLI, not a universal proof of `axiomGate`. -/

namespace Plumb.Qualification.RegistryCli
open Lean System
...
def check : IO Unit := do
  ...
  withScratch root "registry-cli" fun scratch => do
    ...
    for arguments in cases root scratch output do
      ...
      match PlumbQualification.Registry.checkedValidation.run result.exitCode.toNat report with
      | .ok () => pure ()
...
end Plumb.Qualification.RegistryCli
