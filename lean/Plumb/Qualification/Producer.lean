...
build-lint, PL5001/PL5002, Fixed then Violation then restored Fixed) and two
...
PL5001/PL5002 observations are the rule-example corpus records, which that campaign
...
namespace Plumb.Qualification.Producer
open Lean System
...
def check (evidence : Option FilePath) : IO Unit := do
  ...
  let records ← withScratch root "producer-controls" fun scratch => do
    ...
    for (invocation, flags, mode) in #[
      ...
      for rule in #["PL5001", "PL5002"] do
        ...
        for kind in #["Fixed", "Violation", "Fixed"] do
          ...
          IO.ofExcept (PlumbQualification.Producer.checkedValidation.run report result.exitCode.toNat
    ...
    for phase in #["positive", "axiom"] do
      ...
      IO.FS.writeBinFile (project / "Example.lean") (← IO.FS.readBinFile (root / "examples/rules/PL5002/Fixed.lean"))
      ...
      IO.ofExcept (PlumbQualification.checkedDecoded.run
        (PlumbQualification.Producer.standaloneRequirements report result.exitCode.toNat (phase == "axiom")))
...
end Plumb.Qualification.Producer
