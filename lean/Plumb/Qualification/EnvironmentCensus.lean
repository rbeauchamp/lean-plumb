...
namespace Plumb.Qualification.EnvironmentCensus
open Lean System Plumb.Checker PlumbPolicy
...
private unsafe def checkCore (attempt : String) (path : FilePath) : IO Unit := do
  ...
  for modules in expected do
    ...
    atomicWrite packetPath (Json.mkObj [("expectedModules", toJson modules),
      ("report", toJson report), ("transcripts", toJson (#[] : Array PlumbPolicy.Frontend.Transcript)),
    ...
    let mut transcripts : Array PlumbPolicy.Frontend.Transcript := #[]
  ...
  let some leftIndex := (manifest.surfaces.toList.findIdx? (·.library == "PlumbVerification"))
