...
namespace Plumb.Qualification.Preparation
open Lean System

def check : IO Unit := do
  let root ← Plumb.Qualification.rootDirectory
  let scratchRef ← IO.mkRef (none : Option FilePath)
  try
    Plumb.Qualification.withScratch root "prep-measure" fun scratch => do
      ...
      let deps ← Plumb.Checker.Snapshot.dependencies inventory
...
end Plumb.Qualification.Preparation
