...
namespace Plumb.Qualification.FrozenExit
open Lean System PlumbQualification PlumbQualification.Evidence SourceEvidence
  ...
  withScratch root "frozen-exits" fun project => do
    ...
    for (binary, flags) in #[("docFenceAudit", #["--jobs", "1", "--verbose"]), ("axiomGate", #["--with-docs"])] do
      ...
      for phase in #["positive", "changed", "restored"] do
        ...
        check {
          ...
          fence := importSource bad action, ids := if bad then ["PL4002", "PL2005"] else [],
      ...
      check {
        ...
        fence := importSource true action, ids := ["PL4002", "PL2005"],
      ...
      check {
        ...
        fence := importSource true action, ids := ["PL4002"], reason := "checker inspection failed" }
    ...
    for (label, binary, flags) in #[
      ...
      for phase in #["positive", "missing", "restored", "ordinary-error", "ordinary-restored"] do
        ...
        check {
          ...
          ids := if missing then ["PL2005"] else if ordinary then [if binary == "axiomGate" then "PL2003" else "error"] else [],
    ...
    for (binary, flags) in #[("docFenceAudit", #["--jobs", "1"]), ("axiomGate", #["--with-docs"])] do
      check {
        ...
        ids := ["PL4002", "PL2005"], reason := "producer-source: source snapshot unavailable: DocFence_1" }
    ...
    check {
      ...
      ids := ["PL2005"], reason := "producer-source: source snapshot unavailable: AuditFile_" }
    ...
    check {
      ...
      ids := ["PL2005"], reason := "producer-source: configuration snapshot changed:" }
    check { label := "build/configuration-restored" }
end Plumb.Qualification.FrozenExit
