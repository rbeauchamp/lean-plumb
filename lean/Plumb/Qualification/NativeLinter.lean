...
in `PlumbQualification.Native`; actual compiler behavior is observed, not proved. -/

namespace Plumb.Qualification.NativeLinter
open Lean System

/-- The registered claim's docstring, with the nonempty Intent section PL5003 requires. -/
private def claimDoc := "/-- A registered material claim.\n\n# Intent\nQualify the native Intent-section observer. -/\n"

private def base := "import Plumb.Linter\n/-! Collector qualification control. -/\n" ++ claimDoc ++ "@[plumb_material] theorem documented : True := .intro\nprivate def privateValue : Nat := 1\ninductive Branch where\n  | node : List Branch → Branch\n"
...
structure Control where
  ...
  output : Bool := false
  compiler : List PlumbQualification.Native.CompilerMessage := []
...
def decode (value : Json) : Except String PlumbQualification.Native.Message := do
...
private def check (root scratch : FilePath) (launcher : Launcher.State) (control : Control)
  ...
  let expected : PlumbQualification.Native.Expected := {
    kinds := control.ids.map (fun id => s!"Plumb.{id}._namedError"),
  ...
  match PlumbQualification.Native.checkedValidation.run expected result.exitCode.toNat result.stderr decoded with
  | .ok () => pure ()
...
private def position (messages : List Json) (id : String) : IO Json := do
  let some message := messages.find? (fun message =>
    (message.getObjValAs? String "kind").toOption == some s!"Plumb.{id}._namedError")
...
def checkAt (root scratch : FilePath) (launcher : Launcher.State) (jobs : Nat := 4) : IO Unit := do
    ...
    let controls : List Control := [
      ...
      { label := "MissingIntent", source := base.replace claimDoc "/-- A registered material claim. -/\n",
        ids := ["PL5003"] },
      { label := "Disabled", source := base.replace "/-!" "set_option linter.plumb false\n/-!" ++
        "axiom forbidden : False\n" },
      { label := "Narrow", source := base ++ "set_option plumb.localFoundation \"kernel-only\"\n" ++
        "theorem classicalClaim (p : Prop) : p ∨ ¬p := Classical.em p\n", ids := ["PL1005"] },
      { label := "EmptyBadRequest", source := "import Plumb.Linter\n/-! Configuration control. -/\n" ++
        "set_option plumb.localFoundation \"unsupported\"\n", ids := ["PL2002"] },
      { label := "TrailingBadRequest", source := base ++ "set_option plumb.localFoundation \"unsupported\"\n", ids := ["PL2002"] },
      { label := "InitialBadRequest", source := "import Plumb.Linter\n", ids := ["PL2002"],
        options := #["-Dplumb.localFoundation=unsupported"] },
      { label := "ScopedBadRequest", source := base ++ "set_option plumb.localFoundation \"unsupported\" in\n" ++
        "def selected : Nat := 1\n", ids := ["PL2002"] },
      ...
      { label := "RecoveredError", source := base ++ "theorem broken : True := by exact missingProof\n",
        ids := ["PL1002"], errors := true, compiler := [⟨"error", "Unknown identifier `missingProof`"⟩] },
      { label := "Synchronous", source := axiomSource, ids := ["PL1001"], options := #["-DElab.async=false"] },
      ...
      { label := "Contract", source := base ++ "def implementation (n : Nat) : Nat := n\n" ++
        "theorem unsupported (n : Nat) : Plumb.ExecutableContract (implementation n) (fun value => value = n) := ⟨rfl⟩\n",
        ids := ["PL1007"] },
...
def paired : IO Unit := do
  ...
  try
    withScratch root "launcher-pair" fun scratch => do
      ...
      requireChecks [⟨"37 exact paired controls", PlumbQualification.Launcher.checkedEquivalence.run before after⟩]
...
end Plumb.Qualification.NativeLinter
