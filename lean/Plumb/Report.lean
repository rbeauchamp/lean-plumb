...
namespace Plumb.Report
open Lean PlumbPolicy
open Plumb.Checker.PolicyCodec (exactFields)

scoped instance : ToJson Name := ⟨Plumb.RegistryCodec.nameJson⟩
scoped instance : FromJson Name := ⟨Plumb.RegistryCodec.parseName⟩
...
abbrev Position := PlumbPolicy.Position
deriving instance ToJson for PlumbPolicy.Position
instance : FromJson PlumbPolicy.Position := ⟨fun j => do
...
abbrev Range := PlumbPolicy.Range
deriving instance ToJson for PlumbPolicy.Range
instance : FromJson PlumbPolicy.Range := ⟨fun j => do
...
abbrev Ranges := PlumbPolicy.Ranges
deriving instance ToJson for PlumbPolicy.Ranges
instance : FromJson PlumbPolicy.Ranges := ⟨fun j => do
...
abbrev ExecutableContract := PlumbPolicy.ExecutableContract
deriving instance ToJson for PlumbPolicy.ExecutableContract
instance : FromJson PlumbPolicy.ExecutableContract := ⟨fun j => do
...
abbrev Declaration := PlumbPolicy.Declaration
deriving instance ToJson for PlumbPolicy.Declaration
instance : FromJson PlumbPolicy.Declaration := ⟨fun j => do
...
abbrev ExecutionBoundary := PlumbPolicy.ExecutionBoundary
instance : ToJson PlumbPolicy.NativeOrigin := ⟨fun o => Json.mkObj [
  ("module", toJson o.moduleName), ("actual", toJson o.actual), ("expected", toJson o.expected)]⟩
instance : FromJson PlumbPolicy.NativeOrigin := ⟨fun j => do
  exactFields j ["module", "actual", "expected"]
  PlumbPolicy.admitNativeOrigin (← j.getObjValAs? Name "module")
...
abbrev ExecutionRoot := PlumbPolicy.ExecutionRoot
deriving instance ToJson for PlumbPolicy.ExecutionVisit
instance : FromJson PlumbPolicy.ExecutionVisit := ⟨fun j => do
...
deriving instance ToJson for PlumbPolicy.ExecutionClosure
instance : FromJson PlumbPolicy.ExecutionClosure := ⟨fun j => do
...
deriving instance ToJson for PlumbPolicy.ExecutionRoot
instance : FromJson PlumbPolicy.ExecutionRoot := ⟨fun j => do
...
abbrev ModuleOrigin := PlumbPolicy.ModuleOrigin
deriving instance ToJson for PlumbPolicy.ModuleOrigin
instance : FromJson PlumbPolicy.ModuleOrigin := ⟨fun j => do
...
abbrev Environment := PlumbPolicy.Environment
deriving instance ToJson for PlumbPolicy.Environment
instance : FromJson PlumbPolicy.Environment := ⟨fun j => do
...
structure Collected extends PlumbPolicy.Environment where
  census : Census
  deriving Repr
