...
This module is checker infrastructure shipped inside the `Plumb`
...
private def executionWalk (env : Environment) (ownedModules : List Name)
    (nativeModules : NameMap PlumbPolicy.NativeOrigin)
    ...
    (recursorHelpers : Array Name) (root : Name) : CommandElabM (Array Plumb.Report.ExecutionBoundary ×
      Array String × Array (Name × Name) × PlumbPolicy.ExecutionClosure) := do
  let mut visited : Std.HashSet Name := {}
  let mut queue : Array (Name × Option Nat) := #[(root, none)]
  let mut visits : Array PlumbPolicy.ExecutionVisit := #[]
  let mut boundaries : Array Plumb.Report.ExecutionBoundary := #[]
  ...
  while !queue.isEmpty do
    ...
    let entry (boundary : BoundaryKind) (correspondence : Correspondence) (replacement : Option Name) (evidence : Option String) :
        CommandElabM Plumb.Report.ExecutionBoundary := do
      let account ← match PlumbPolicy.admitBoundaryEvidence boundary correspondence evidence
  ...
  let closure : PlumbPolicy.ExecutionClosure := {
    nodes := PlumbPolicy.canonicalNames visited.toArray
    visits
    logicalEdges := PlumbPolicy.canonicalEdges logicalEdges
    candidateEdges := PlumbPolicy.canonicalEdges candidateEdges
    historyEdges := PlumbPolicy.canonicalEdges historyEdges
    currentReplacementEdges := PlumbPolicy.canonicalEdges currentReplacementEdges
    activeSimplificationEdges := PlumbPolicy.canonicalEdges activeSimplificationEdges
    helperEdges := PlumbPolicy.canonicalEdges helperEdges
    requiredCode := PlumbPolicy.canonicalNames compiledNames.toArray
    unavailableCode := PlumbPolicy.canonicalNames unavailableCode
  }
  return (boundaries, unresolved, PlumbPolicy.canonicalEdges compilerEdges, closure)
...
private def executableRoots (env : Environment) (own : Array (Name × ConstantInfo)) :
  ...
  for (name, info) in own do
    ...
    | .defnInfo _ | .opaqueInfo _ =>
        ...
        let typeProducing ← liftTermElabM <| Plumb.Collect.returnsSort info.type
...
def environmentReport (modules : List Name)
    ...
    CommandElabM Plumb.Report.Collected := do
  ...
  let entries ← own.mapM fun (name, _) => Plumb.Collect.declaration name .replayCandidate
  ...
  let execution ← if includeExecution then do
    ...
    let mut nativeModules : NameMap PlumbPolicy.NativeOrigin := {}
      ...
      if moduleName.getRoot == `Init then
        ...
        if ← liftIO expected.pathExists then
          if actual == (← liftIO <| IO.FS.realPath expected) then
            let receipt ← match PlumbPolicy.admitNativeOrigin moduleName actual.toString
    ...
    roots.mapM fun (moduleName, root) => do
      ...
      return ({
        ...
        Plumb.Report.ExecutionRoot)
    else pure #[]
  return {
    toolchain := Lean.versionString
    modules := PlumbPolicy.canonicalNames env.header.moduleNames
    moduleOrigins
