...
public meta import Plumb.Linter.Rules
public meta import Plumb.Linter.Documentation
...
register_option linter.plumb : Bool := {
  defValue := true
  descr := "Emit Plumb local feedback; project checking remains required." }
register_option plumb.localFoundation : String := {
...
private def localRequest (stx : Syntax) : CommandElabM (Option PlumbPolicy.InspectionRequest) := do
  match Rules.request (plumb.localFoundation.get (← getOptions)) with
...
initialize addLinter {
  name := `Plumb.Linter.declarations
...
initialize addModuleLinter {
  name := `Plumb.Linter.documentation
  run := fun commands => withRef (commands.back?.getD Syntax.missing) do
    ...
    if (Rules.request (plumb.localFoundation.get (← getOptions))).toOption.isNone then
