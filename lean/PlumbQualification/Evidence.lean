...
namespace PlumbQualification.Evidence
open Lean
...
def requirements (expected : Expected) (code : Nat) (transcript : String)
  ...
  let mut checks := [
    ...
    ⟨"no invented evidence refusal", expected.ids.contains "PL2005" || !(transcript.contains "PL2005")⟩]
  if let some result := result then
    ...
    for diagnostic in ← array result "diagnostics" do
      ...
      if (← text diagnostic "id") == "PL2005" then
        checks := checks ++ [
...
def documentationChecks (transcript scope reason phase : String) : List Check :=
  ...
  let admission := lines.filter (·.startsWith "PL2005 [")
  ...
  [⟨"snapshot refusal", transcript.contains reason⟩,
   ...
   ⟨"one fence compilation diagnostic", (lines.filter (·.startsWith "PL4002 [")).length == 1⟩,
...
theorem checkedDocumentation : Plumb.ExecutableContract validateDocumentation
...
end PlumbQualification.Evidence
