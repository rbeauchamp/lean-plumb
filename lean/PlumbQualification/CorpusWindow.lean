...
driver (`Plumb.Qualification.RuleExamples.check`) launches exactly the jobs these
...
namespace PlumbQualification.CorpusWindow
...
def specials (selected : List String) : List (String × String) :=
  (if "PL1005" ∈ selected then [("PL1005", "WrongClaim")] else []) ++
    (if "PL4004" ∈ selected then [("PL4004", "TrustedControl"), ("PL4004", "NegativeControl")]
      else [])
...
end PlumbQualification.CorpusWindow
