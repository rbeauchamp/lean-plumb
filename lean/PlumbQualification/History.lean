...
namespace PlumbQualification.History
open Lean PlumbPolicy.Guards
...
def requirements (report : Json) (code : Nat) (mode source : String)
  ...
  let base : List Check := [
    ...
    ⟨"history diagnostics", ids.eraseDups == (if unsupported then ["PL3001"] else [])⟩,
...
end PlumbQualification.History
