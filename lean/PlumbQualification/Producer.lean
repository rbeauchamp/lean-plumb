...
/-! Exact producer-observation requirements for PL5001/PL5002 and standalone roots.
...
namespace PlumbQualification.Producer
open Lean
...
def requirements (report : Json) (code : Nat) (rule mode source : String)
  ...
  let mut checks : List Check := [
    ...
    ⟨"module documentation", (← field docs "modules") == toJson #[toJson #[nameJson "Example", .bool (rule == "PL5002" || fixed)]]⟩,
  ...
  if !fixed then
    ...
    checks := checks ++ [
      ...
      ⟨"location kind", (← text location "kind") == (if rule == "PL5001" then "module" else "source")⟩,
...
def standaloneRequirements (report : Json) (code : Nat) (mutated : Bool) : Except String (List Check) := do
  ...
  return [
    ...
    ⟨"standalone diagnostics", ids == (if mutated then ["PL1001"] else [])⟩,
...
end PlumbQualification.Producer
