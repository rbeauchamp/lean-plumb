...
namespace PlumbQualification.Native
...
/-- Plumb owns precisely the native named-error prefix used by the old harness. -/
def isNative (message : Message) : Bool := message.kind.startsWith "Plumb.PL"
...
def nativeMatches (expected : Expected) (message : Message) : Bool :=
  ...
  message.data.contains "https://rbeauchamp.github.io/lean-plumb/dev/rules/" &&
...
def NativeMatches (expected : Expected) (message : Message) : Prop :=
  ...
  message.data.contains "https://rbeauchamp.github.io/lean-plumb/dev/rules/" = true ∧
  message.severity = expected.severity
...
theorem validate_exact (expected : Expected) (exitCode : Nat) (stderr : String)
  ...
  simp [validate, Plumb.ExecutableContract.run, evaluate_success, Satisfied,
...
theorem checkedValidation : Plumb.ExecutableContract validate
