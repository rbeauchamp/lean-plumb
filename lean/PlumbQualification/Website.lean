...
namespace PlumbQualification.Website
...
theorem checkedBlock : Plumb.ExecutableContract block
...
def renderPage (input : PageInput) : Except String (Page input) :=
  if h : FenceFree input ∧ input.id = "PL1001" then
    .ok ⟨pageText input, rfl, h⟩
  else .error "prototype requires PL1001 and fence-free checked text"

/-- All and only supported fence-free inputs produce the exact specified document. -/
theorem renderPage_exact (input : PageInput) :
    (renderPage input).isOk = true ↔ FenceFree input ∧ input.id = "PL1001" := by
...
theorem checkedPage : Plumb.ExecutableContract renderPage
    (fun run => ∀ input, (run input).isOk = true ↔ FenceFree input ∧ input.id = "PL1001") :=
  ⟨renderPage_exact⟩

/-- Non-vacuity of page admission with actual nonempty source sections. -/
theorem page_positive_control :
    (renderPage ⟨"PL1001", "Axiom", "docs/standard/8-tooling-and-machine-audit.md", "x", "d", "y"⟩).isOk = true := rfl

/-- A different rule cannot reuse the fixed PL1001 narrative. -/
theorem page_wrong_rule_control :
    (renderPage ⟨"PL5001", "Docs", "", "x", "d", "y"⟩).isOk = false := rfl
...
theorem checkedArtifact : Plumb.ExecutableContract siteArtifact
    (fun run => ∀ input, ArtifactMatches input (run input)) := ⟨artifact_exact⟩

end PlumbQualification.Website
