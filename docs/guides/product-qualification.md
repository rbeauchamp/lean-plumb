...
compliant example, with the order and once-per-rule properties proved there.
...
| `lake lint -- --json-out PATH` | Same audit, result schema 3 | `status`, `complete`, diagnostics in run order with source ranges, `remedy` and `helpUrl`, and each fired rule's guidance (`rules`) | `ResultProtocol.admitGuidance` on every rule-example result; observed |
