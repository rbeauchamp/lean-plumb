...
| --- | --- | --- |
| `Checker/Policy.admitScope` | `checkedScope` (`ScopeContract`, for every coordinate check; the adapter supplies `Frontend.validateCoordinates`): first coordinate refusal in transcript order, then exactly `admitInventory` with `authorize`; success iff all coordinate checks and `InventoryValid` hold, retaining both input arrays | The coordinate check itself (Lean `FileMap` over transcript bytes), actual source and compiler observation acquisition. |
...
| `AxiomGate.auditSurfaceAt`, `FreshChecker`, file gate (via `Checker/Acceptance`) | `checkedSurfaceAssignments` (`SurfaceAssignmentsContract`), `checkedConformingProfile` (`ConformingProfileContract`) in claimed `StrictLeanCore.Assembly` | Manifest parsing and Lake loading; the contracts concern the decoded records. |
