...
| `ruleExamples --injected-git-facts FACTS [axiomGate] ARGS`, internal corpus-producer entry only | The same frozen inputs as the wrapped row above, except that for a dependency whose exact capture request (package, canonical root, ordered source and configuration paths) matches an entry in FACTS, the Git revision and dirty bit come from the corpus runner's single capture instead of this process. Every source and configuration byte is still read here. | Same carried records as the wrapped row | Same terminal checks. The runner protects the shared dependency roots against user writes, captures once, requires equal content identity and reruns `Snapshot.inputsUnchanged` on that capture at run end. User-facing `axiomGate` rejects the flag. |
