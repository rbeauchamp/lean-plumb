...
- the base/helper mutual-group lists map exactly through Lean's `._unsafe_rec` transformation, and the helper's entire value is exactly Lean 4.33.1's pinned recursive-constant replacement of the built-in structural or well-founded predefinition stored for the base;
