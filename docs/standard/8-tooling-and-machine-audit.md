...
- the checker executables (`axiomGate`, `docFenceAudit`, `freshChecker`, `checkerSelftest`) are discovered as root-package `lean_exe` targets and recorded in `excluded-executables`: they are operational tooling whose root modules belong to the excluded `Plumb` library, qualified by the §8.8 mutation suite rather than claimed as a proof surface;
