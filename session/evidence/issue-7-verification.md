...
- **Issue 7 remains open.** Neither issue 7 as a whole nor its build-lint/incremental claim is closed by this record.
- **Merged PRs, not a completed delivery:** https://github.com/rbeauchamp/strict-lean/pull/33 (main `12a704502f78d578ccfd81ec7c93bc0dd88af757`) and the review follow-up https://github.com/rbeauchamp/strict-lean/pull/45 (main `b0f1ccb83596b703c1bf2a3ce5502642f417b1ee`) are merged. Exact-head hosted CI and post-merge main CI and Diagnostics were green for both. Those workflows do not run the build-integration diagnostic.
- **Blocking failure:** the changed build-integration capability's `diagnostics environments` and `diagnostics build-policy` partitions both FAILED at the hard 420 s deadline at the closeout head (last section of this file). Per `AGENTS.md`, a `FAIL` blocks the affected claim, so the build-lint/incremental claim is blocked. The fourth deliverable (documentation fences and build-lint's incremental claim, with the changed build-integration diagnostic) and the fifth acceptance criterion (common DoD) stay open.
- **Separate follow-up:** the adopter-facing dependency-capture cost is the per-run, Mathlib-wide dependency capture that issue 7 added to every `axiomGate` route, including `--build-lint`. It is separate follow-up work that Firstmate is filing. No diagnostic sharding stopgap is adopted.
...
| `unknown` (evidence) | Unknown category | In process, a declaration kind is a value of a Lean inductive, so an unknown kind cannot be represented. For serialized worker packets, the derived `FromJson` decoders refuse unknown constructors. That is the pinned derive handler, a trusted boundary: no executed control exercises unknown-constructor refusal by a derived `FromJson` decoder. `process-overflow` in the transport controls covers only the hand-written exit-code range refusal. No executed mutation specific to unknown declaration kinds remains; this matches cut 3's "parent re-check of serialized evidence (vacuous once nothing crosses a process boundary)". |
...
- **environments:** exit 1 (SIGKILL), 420.06 s, 01:09:49–01:16:50 UTC. Completed: the clean-checkout phases and the `external adopters` phase (173.9 s). In progress at the kill: clean-checkout/fresh-checker.
- **build-policy:** exit 1 (SIGKILL), 420.01 s, 01:16:50–01:23:50 UTC. Completed: 26 of its 27 build-lint controls (positive / intended mutation / fresh restoration each). In progress at the kill: `reenabled-cached-axiom`; the final verdict did not complete.
...
- **Receipts and logs:** written locally to `tmp/closeout-96ed742-{environments,build-policy}.{receipt,log}`. `tmp/` is gitignored and these files are not retained in the repository; the key receipt fields are recorded inline above.

This is a genuine failure of the changed build-integration capability's diagnostic within its limit, not a pass. Per `AGENTS.md`, this `FAIL` blocks the build-lint/incremental claim, and issue 7 remains open. The adopter-facing dependency-capture cost (per-run, Mathlib-wide dependency capture added to every `axiomGate` route, including `--build-lint`) is separate follow-up work that Firstmate is filing. No diagnostic sharding stopgap is adopted.
